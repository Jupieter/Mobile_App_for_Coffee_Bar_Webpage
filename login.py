...
class LogInCard(MDCard): # the.boss@staff.com    Enter1
	...
	def log_out(self):
		...
		store = requests.post('http://jupieter.pythonanywhere.com/c_app/logout/', headers=hd_token)
		print(hd_token)
		print(store)
	...
	def log_in(self):
		...
		if user != "" or password != "" :
			...
			sends = x
			print(sends)
			store = requests.post('http://jupieter.pythonanywhere.com/c_app/login/', data=sends).json()
			print(store)
			keys = []
			values = []
